...
.*  Note
.*----------------------------------------------------------
:h1 id=BUGBUG.ATTENTION&colon. important note for WARP users
:p.:hp8.The OS/2 workplace shell of OS/2 Warp 3.0/4.0 has an annoying bug&colon.
:p.If the name of the workplace shell including its absolute path is too long, the desktop crashes during
startup.
If e.g. the desktops are located inside :font facename=Courier size=14x14.\WPSAC\DESKTOPS:font size=0x0.,
the name of a desktop must have a length of at most 10 characters!
:ehp8.
...
:p.:hp2.WPSac:ehp2. is an access control software for the OS/2 workplace shell.
It allows the administration by an administrator, who sets up user specific authorizations.
Each user of the PC may either get his own desktop or different desktops may be shared among
different users.

:p.There are a lot of applications for the :hp2.WPSac:ehp2.. In the simplest case it may be used to
offer different desktops for different users. In many cases any access restrictions for single objects
are not necessary.

:p.The access rights to objects on the different desktops may be managed for each user by means of
:link reftype=hd refid=UPROFPRF.access profiles:elink..
The combination of several users to groups is also possible.

:p.The purpose of the access profile is it, to suppress some functionalities of the objects on a desktop
(e. g. to prevent unpractised users of the destruction of these objects).

:p.Finally the administrator is able to configure the behaviour and outlook of the desktops all over the 
network. Changes to the desktops or access profiles may also be introduced later on.

:p.The different users may be protected by passwords, which may alternatively be entered by a
:link reftype=hd refid=MAGSUPP.magnetic card reader:elink.

:p.Some important features are&colon.
:ul compact.
:li.A graphical administration program to administrate users, desktops and access profiles.
Each user may own a password, an access profile or may share a group profile.
:li.Access profiles may be created by means of an ASCII file and compiled with a special 
compiler or alternatively created grafically with the administration program.
:li.The administration of a user may be done via LAN by the administrator. :hp2.WPSac:ehp2 
supports both IBM LAN-Server and Novell Netware.
...
:li.If the network does not work, each PC may be operated stand alone with the last valid profiles.
:li.The logon procedure of a user may also include the logon for networks of databases.
(:link reftype=hd refid=UPROFLOG.single signon:elink. feature).
:eul.
...
:xmp.
   ...
   4.0  UnterstÅtzung von IBM LAN-Server, Erweiterung von SAMMY  
